...
const TableRows = ({desc,word,obj,addtoarr}) => {
  ...
  const HandleChangeVatVal=(e)=>{
   ...
   return                 <input  className={`${desc}vr-input`} min={1} onChange={
    (e)=>{ addtoarr(e.target.val);console.log(obj)}
   }  
  ...
  //  vatVal/120*20
   ...
   if(word=='sales'){
    return ( 
       ...
       <tr>  
            <td>
                <input  className={`${desc}v-input`} min={1} onChange={(e)=>handleChangeVat(e)}  defaultValue={0} onClick={(e)=>handleClick(e)} type="number"/>
            </td>
            <td>
                <HandleChangeVatVal/>
